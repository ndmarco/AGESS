...
for i in 1:25
  ...
  if i == 1
    Stan_β = plot(df[1:10:end, findall(β .!= 0)], legend = false, dpi = 300, ylims=[-2.5,4.5])
  ...
  if i == 1
    ...
    GESS_β = plot(x_GESS[250000:10:end, findall(β .!= 0)], legend = false, dpi = 300, ylims=[-2.5,4.5])
    hline!(β[findall(β .!= 0)], line = :dash, color =:black)
  ...
  ess_HS
  ess_Stan <- NA
  try(ess_AGESS <- n.eff(x_AGESS1)$n.eff)
  try(ess_Stan <- n.eff(x_stan1)$n.eff)
  try(ess_HS <- n.eff(x_HS1)$n.eff)
  """
  @rget ess_Stan
  @rget ess_HS
  @rget ess_AGESS
